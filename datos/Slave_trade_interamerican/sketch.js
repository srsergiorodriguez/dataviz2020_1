...
d3.csv("interamerican_traffic.csv",(element)=> {
  ...
  // embark_bars(data);
  // disembark_bars(data);
  // slave_arrival_bars(data);
  // yearxresistance_scatter(data);
  // yearxslavesfate_scatter(data);
  // yearxownerfate_scatter(data);
...
function slave_arrival_bars(data) {
  let new_data = {};
  data.filter(d=>d.lugar_llegada_esclavos!=undefined).map(d => {
    if (new_data[d.lugar_llegada_esclavos]) {
      new_data[d.lugar_llegada_esclavos].count++;
    } else {
      new_data[d.lugar_llegada_esclavos] = {
        label: d.lugar_llegada_esclavos,
        count: 1
      }
    }
  });
  ...
  svg.append("g")
    .attr("transform", `translate(${margin.l},${margin.t/2})`)
    .append("text")
    .text(`Países de llegada de los esclavos. (Más de ${num_casos} barcos)`)
...
function yearxresistance_scatter(data) {
  let svg = d3.select("#svg_graph")
  
  svg.append("g")
    .attr("transform",`translate(${margin.l},${margin.t/2})`)
    .append("text")
    .text("Año x eventos de resistencia de los esclavos")
  
  let tp = d3.timeParse("%d/%m/%Y")

  let x_scale = d3.scaleTime()
    .domain([new Date("1550-01-01"), new Date("1860-01-01")])
    .range([0,viz_w])    
  ...
  let y_scale = d3.scalePoint()
    .domain(data.map(d=>d.resistencia))
    .range([viz_h,0])
    .padding(1)
  ...
  let points = svg.append("g")
    ...
    .enter()
    .append("g")
    .attr("transform",d=>`translate(${x_scale(tp(d.fecha_llegada_esclavos))},${y_scale(d.resistencia)})`)
    .append("circle")
    .attr("cx",0)
    .attr("cy",0)
    .attr("r",10)
...
function yearxslavesfate_scatter(data) {
  let svg = d3.select("#svg_graph")
  
  svg.append("g")
    .attr("transform",`translate(${margin.l},${margin.t/2})`)
    .append("text")
    .text("Año x suerte de los esclavos")
  ...
  let y_scale = d3.scalePoint()
    .domain(data.map(d=>d.suerte_esclavos))
    .range([viz_h,0])
    .padding(1)
  ...
  let points = svg.append("g")
    ...
    .enter()
    .append("g")
    .attr("transform",d=>`translate(${x_scale(tp(d.fecha_llegada_esclavos))},${y_scale(d.suerte_esclavos)})`)
    .append("circle")
    .attr("cx",0)
    .attr("cy",0)
    .attr("r",10)
...
function yearxownerfate_scatter(data) {
  let svg = d3.select("#svg_graph")
  ...
  let y_scale = d3.scalePoint()
    .domain(data.map(d=>d.suerte_dueno))
    .range([viz_h,0])
    .padding(1)
  ...
  let points = svg.append("g")
    ...
    .enter()
    .append("g")
    .attr("transform",d=>`translate(${x_scale(tp(d.fecha_llegada_esclavos))},${y_scale(d.suerte_dueno)})`)
    .append("circle")
    .attr("cx",0)
    .attr("cy",0)
    .attr("r",10)
...
function departxarrival_parallel(data) {
  ...
  let y2_scale = d3.scalePoint()
    .domain([... new Set(data.map(d=>d.region_llegada))])
    .range([0,viz_h])
  ...
  let lines = svg.append("g")
    ...
    .attr("y2",d=>y2_scale(d.region_llegada))

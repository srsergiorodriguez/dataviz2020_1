...
let anchoLinea = alto - (margenAbj + margenArr + 460);
...
function titulo () {
  ...
  textSize(fuenteTamano - 15);
  text("...algunos datos sobre mí...", ancho/2, margenArr + 36);
}
...
function inicioFin( x, y) {
  ...
  text(annoFinal, ancho - margenDer +30, y);
  textSize(fuenteTamano - 9);
  text("Design by Mónica Ruiz", ancho /2, alto - 5);
  text("* Con el mouse sobre la mascota...  conocerás su edad", x + 50, y + 155);
  textStyle(NORMAL);
}
...
function mascotas (x,y) {
  ...
  for (let i = 0; i < mrm.mascotas.length; i++) {
    ...
    fill(231, 76, 60);
    stroke(231, 76, 60);
    mascotaEdad = mrm.mascotas[i].edad;
    tamanoImagen =  (mascotaEdad*30)/tamanoMax;
    if (tipo == 'perro'){
      image(imgPerro, posicionX - 15, y - 65,tamanoImagen,tamanoImagen);
    } else {
      image(imgGato, posicionX - 15, y - 65,tamanoImagen + 10,tamanoImagen + 10);
    }
    circle(posicionX, y1Linea + (anchoLinea/2), 8);
    textSize(fuenteTamano -3);
...
function familia (x,y) {
  ...
  textFont(fuente);
  textSize(fuenteTamano);
  text("Mi familia...", x - 70, y - margenArr + 50);
  textAlign(CENTER);
  for (let i = 0; i < mrm.sobrinos + 1 ; i++) {
    ...
    stroke(126);
    line(posicionX + 10, y - margenArr + 50, posicionX + 10, y1Linea + (anchoLinea/2));
    fill(243, 156, 18);
    stroke(243, 156, 18);
    circle(posicionX + 10, y1Linea + (anchoLinea/2), 8);
    if (tipo == 'hermana' || mrm.datos_sobrinos[i-1][1]=="M"){
      image(imgMujer, posicionX - 5, y - margenArr + 20,30,30);
    } else {
      image(imgHombre, posicionX - 5,y - margenArr + 20,30,30);
    }
    textSize(fuenteTamano -5);
    text(nombre,posicionX + 5,y - margenArr + 15);
    textSize(fuenteTamano-6);
    text(anno,posicionX + 10,y1Linea + (anchoLinea/2) + 20);
  }
}

// Pintar mis características
...
function setup() {
  ...
  miCanvas.background(255,255,255);
  annoFinal = year();
  // Encabezado
  fill (212,230,241);
  noStroke();
  rect(0,0,ancho,alto - 600, 20);
  ...
  caracteristicas(margenIzq + 80, alto - (margenAbj + 50));
